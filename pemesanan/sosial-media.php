...
require '../lib/session_user.php';
	...
	require '../lib/session_login.php';
		...
		if (!$post_target || !$post_layanan || !$post_kategori) {
			...
			if ($provider == "MANUAL") {
				$api_postdata = "pksMKj8kCKePD132nvmR4mri";
			} else if ($provider == "JAGOSOSMED") {
			    if ($post_comments == false) {
                $postdata = "api_key=".$data_provider['api_key']."&action=order&service=".$data_layanan['provider_id']."&data=$post_target&quantity=$post_jumlah";
			    } else if ($post_comments == true) {
			    $postdata = "api_key=".$data_provider['api_key']."&action=order&service=".$data_layanan['provider_id']."&data=$post_target&quantity=$post_jumlah&custom_comments=$post_commets";
			    }
			} else {
				die("System Error!");
			}
